...
-- hepsini almak icin (LEFT JOIN)
  ...
  -- shorter version (LEFT JOIN)
  SELECT 	m.marka_id, m.marka_adi, 
			s.siparis_adedi, s.siparis_tarihi
  FROM markalar m -- tablolara alias verirken as kullanmiyoruz, sutunlarda 'as' kullaniyoruz
...
SELECT g.name AS genre_of_song, t.name AS song_name
FROM tracks t
JOIN genres g
...
SELECT tracks.TrackId, tracks.name, tracks.AlbumId
FROM tracks
WHERE tracks.AlbumId = 88; -- this is not dynamic, it's a hard-coded method
...
FROM tracks t
JOIN albums a
ON t.AlbumId = a.AlbumId AND a.Title = 'Faceless'; -- instead of WHERE we used AND 
